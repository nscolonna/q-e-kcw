...
# printf "$i "
 ii=`echo $i*4 | bc`
 grep "KI  " results/SPIN_UP/cri3.kcw-ham.out   | head -$ii | tail -4 | sed -e "s/KI//" | xargs > pp
 grep "KI  " reference/SPIN_UP/cri3.kcw-ham.out | head -$ii | tail -4 | sed -e "s/KI//" | xargs >> pp
...
# printf "$i "
 ii=`echo $i*4 | bc`
 grep "KI  " results/SPIN_DOWN/cri3.kcw-ham.out   | head -$ii | tail -4 | sed -e "s/KI//" | xargs > pp
 grep "KI  " reference/SPIN_DOWN/cri3.kcw-ham.out | head -$ii | tail -4 | sed -e "s/KI//" | xargs >> pp
 for j in `seq $nstart 42`; do
  col=$j
  eig=`head -1 pp | awk -v col=$col '{print $col}'`
  eig_ref=`tail -1 pp | awk -v col=$col '{print $col}'`

...
check=1
for i in `seq 1 8`; do
    a=`grep relaxed results/kc.kso | head -$i | tail -1|  awk '{print $12}'`
a_ref=`grep relaxed reference/kc.kso | head -$i | tail -1 | awk '{print $12}'`
err=`echo $i $a $a_ref | awk '{printf "%20.15f \n", sqrt(($2-$3)*($2-$3))}'`
...
for i in `seq 1 8`; do 
 grep "KI  " results/kc.kho | head -$i | tail -1 > pp
 grep "KI  " reference/kc.kho | head -$i | tail -1 >> pp
 for j in `seq 1 8`; do 
  col=`echo $j+1 | bc`

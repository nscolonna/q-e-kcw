...
SUBROUTINE compute_map_ikq_single (iq,also_minus)
  ...
  USE control_kcw,          ONLY : map_ikq, shift_1bz, kcw_iverbosity, nkstot_eff, &
                                   map_ikq_minus, shift_1bz_minus, x_q
...
SUBROUTINE find_index_1bz_iterate(xkq, gvect, ikq)
  ...
  !! once pools are active, while x_q holds the full k/q mesh on every process
  !! (see compute_map_ikq_single above).

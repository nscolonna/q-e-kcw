...
SUBROUTINE rotate_ks ()
  ...
  !
  INTEGER :: h_dim
  ! Dimension of the Hamiltonian diagonalized in ks_hamiltonian: fixed for the whole
  ! loop (kcw_at_ks does not vary with ik), nbnd if kcw_at_ks else num_wann
  ...
  ! argument (see the note there) even when check_ks is off.
  ...
  k_loop: DO ik = 1, nks
     ...
     IF (check_ks) THEN
       ...
       et_wann_chk(:,ik_eff)  = eigvl_wann_chk(1:h_dim)

...
Epsilon.x only supports the reduction of the k-points grid into the irreducible Brillouin zone for \emph{eps} 
calculations. When computing other quantities the previous PW runs must be
...
coordinates must be given manually in \emph{crystal} or \emph{alat} or \emph{bohr}, but not with the \emph{automatic} option. Since the
auto-symmetrization of k-points grid can produce a non uniform distribution of k-points weights, in order to avoid this
PW's behavior the variable NOSYM must be set .TRUE. disabling auto-symmetrization for anything other than \emph{eps} calculations.
\section{Input file}
...
In all the cases illustrated above the non-local contribution $V_{nl}$ due to the pseudopotential is neglected. In particular,
a contribution $\propto [\hat V_{nl},\hat r]$ should be present since we are working in the velocity gauge. This has been
been implemented for \texttt{eps_calc}. The off-diagonal \texttt{offdiag_calc} however does not have this feature yet.\\

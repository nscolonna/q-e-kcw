...
script first. \configure\ will (try to) detect available compilers and libraries for
...
\configure\ supports out-of-source builds, ie allows to compile the binaries in
...
instructions. Symlinks to executable programs appear in the \texttt{build/bin/} 
subdirectory.
\texttt{NB:} the usage of a \textt{build} directory, even though advised for, 
...
\noindent \configure\ generates the following files in the build directory:

\begin{tabular}{ll}
\texttt{make.inc} &      compilation rules and flags (used by \texttt{Makefile})\\
\texttt{configure.msg} & a report of the configuration run (not needed for compilation)\\
\texttt{config.log} & detailed log of the configuration run (useful for debugging)\\
\end{tabular}\\

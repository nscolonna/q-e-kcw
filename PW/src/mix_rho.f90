...
END SUBROUTINE mix_rho
 ...
 SUBROUTINE assign_scf_to_mix_type( rho_s, rho_m )
  ...
  !$acc exit data delete(rho_s, rho_s%of_g)  
   ...
   IF (okpaw)  THEN
      IF (nspin == 2 ) THEN
           rho_m%bec(:,:,1)     = 0.5_dp * (rho_s%bec(:,:,1) + rho_s%bec(:,:,2))
           rho_m%bec(:,:,2)     = 0.5_dp * (rho_s%bec(:,:,1) - rho_s%bec(:,:,2))
      ELSE
          rho_m%bec = rho_s%bec
      END IF
   END IF
 ...
 SUBROUTINE assign_mix_to_scf_type( rho_m, rho_s )
  ...
  !$acc exit data delete(rho_s) 
   ...
   IF (okpaw)  THEN
     IF (nspin == 2 ) THEN
       rho_s%bec(:,:,1) = rho_m%bec(:,:,1) + rho_m%bec(:,:,2)
       rho_s%bec(:,:,2) = rho_m%bec(:,:,1) - rho_m%bec(:,:,2)
     ELSE
       rho_s%bec = rho_m%bec
     END IF
   END IF
...
FUNCTION rho_ddot( rho1, rho2, gf, g0, spinstop )
  ...
  INTEGER, OPTIONAL, INTENT(IN) :: spinstop
  !! (imix==3) restrict the magnetization cross term to spin channels 2:spinstop
  REAL(DP) :: rho_ddot
